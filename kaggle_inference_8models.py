#!/usr/bin/env python3
"""
AfriKnow — Kaggle OpenRouter Inference Script (8 models, 180 mixed-source items)

This script is designed to run on Kaggle notebooks with OpenRouter API access.
It replaces the local laptop execution to avoid battery drain and network issues.

Usage on Kaggle:
    !python kaggle_inference_8models.py

OpenRouter API key must be set as a Kaggle secret (Add-ons → Secrets → OPENROUTER_API_KEY).
"""
...
# Add repo root to path so we can import shared modules
REPO_ROOT = Path("/kaggle/input/afriknow-code") if Path("/kaggle/input/afriknow-code").exists() else Path(".").resolve()
...
OUT_DIR = Path(".")
ITEMS_JSON = OUT_DIR / "02_mixed_source_180_items.json"
...
# ALL 8 MODELS: 5 current + 3 reserve
MODELS = [
...
def get_openrouter_client():
    """Initialize OpenRouter client from environment or Kaggle secret."""
...
def chat_complete(client, model_id: str, prompt: str, temperature: float, max_tokens: int | None, purpose: str, cost_tracker: CostTracker) -> tuple[str, int, int]:
    """Call OpenRouter API with retries and cost tracking. Returns (content, input_tokens, output_tokens)."""
    cost_tracker.check()
...
def main():
    ...
    parser.add_argument("--n-items", type=int, default=None, help="Override number of items")
    parser.add_argument("--cost-cap", type=float, default=COST_CAP_USD)
    parser.add_argument("--smoke", action="store_true", help="1 item per model")
    ...
    # Preflight: test all models with first item
    active = []
    ...
    if not active:
        raise RuntimeError("No models passed preflight. Check API key and network.")
    print(f"[kaggle] Active models ({len(active)}): {[m[1] for m in active]}")

    # Override MODELS with active list
    active_models = active
    print(f"[kaggle] Active models ({len(active_models)}): {[m[1] for m in active_models]}")

    # Run inference
    t0 = time.time()
    results = run_inference(items, client, cost_tracker, active_models)
    dt = time.time() - t0
    ...
    manifest = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "script": "kaggle_inference_8models.py",
        "provider": "openrouter",
        "seed": SEED,
