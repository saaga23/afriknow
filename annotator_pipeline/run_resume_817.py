#!/usr/bin/env python3
"""
Resume runner for full 817-item expansion.
Uses items 621-817 from the full dataset.
Appends results to existing checkpoint/output files.
"""
...
# Resume dataset: items 621-817 (197 items)
SAMPLED_JSON = OUT_DIR / "02_sampled_items_resume.json"
RESUME_OFFSET = 620  # items 0-620 already done
...
def log(msg: str) -> None:
    print(f"[resume-817] {msg}", flush=True)
...
class CostTracker:
    ...
    def add(self, model, it, ot, purpose):
        c = self.estimate(model, it, ot)
        self.spent += c
        self.history.append({
            "model": model, "purpose": purpose, "input_tokens": it,
...
def main():
    ...
    log(f"Loaded {len(items)} resume items from {SAMPLED_JSON.name}")
    log(f"Item indices will be offset by +{RESUME_OFFSET}")
    ...
    if DRY_RUN:
        ...
        for idx, it in enumerate(items):
            for model_id, nick, temp, max_tok in active:
                ...
                base = {
                    "item_idx": idx + RESUME_OFFSET,
                    "id": it.get("id"), "qid": it.get("qid", it.get("id")),
                ...
                results.append({**base, "purpose":"greedy", "pred":pred,
                                "correct":int(pred==it.get("answer","A")),
                                "vce":None, "sc_agree":None, "cocoa_fixed":None, "greedy_text":"dry-run"})
                results.append({**base, "purpose":"vce", "pred":pred,
                                "correct":int(pred==it.get("answer","A")),
                                "vce":vce, "sc_agree":None, "cocoa_fixed":0.5 * vce + 0.5, "greedy_text":"dry-run"})
        df = pd.DataFrame(results)
        df.to_csv(OUT_DIR / "03_openrouter_outputs_resume.csv", index=False)
        log(f"Saved 03_openrouter_outputs_resume.csv ({len(df)} rows, dry-run)")
        return
        ...
        for model_id, nick, temp, max_tok in active:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    g_ok = True
                    base = {
                        "item_idx": global_idx, "id": item.get("id"), "qid": item.get("qid", item.get("id")),
                    ...
                    results.append({**base, "purpose":"greedy", "pred":pred,
                                    "correct":int(pred==item.get("answer","A")),
                                    "vce":None, "sc_agree":None, "cocoa_fixed":None,
                                    "greedy_text":clean_raw_text(c, "greedy")})
                    break
                except Exception as e:
                    log(f"  {nick} item {global_idx} greedy attempt {attempt+1}: {e}")
                    failures[nick] += 1
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    results.append({**base, "purpose":"vce", "pred":results[-1]["pred"],
                                    ...
                                    "greedy_text":clean_raw_text(vc, "vce")})
                    break
                except Exception as e:
                    log(f"  {nick} item {global_idx} VCE attempt {attempt+1}: {e}")
                    failures[nick] += 1
                    time.sleep(BACKOFF * (attempt+1))
            if not v_ok:
                log(f"  {nick} item {global_idx}: VCE FAILED")
            if ((global_idx + 1) % CHECKPOINT_EVERY == 0) or (global_idx == 816):
                pd.DataFrame(results).to_csv(OUT_DIR / f"03_or_checkpoint_{nick}_{global_idx+1}.csv", index=False)
            cost_tracker.check()
            if failures[nick] / max(1, sum(failures.values())) > MAX_FAILURE_RATE:
                isolated.add(nick)
                log(f"Model {nick} isolated.")

    df = pd.DataFrame(results)
    df = df[REQUIRED_COLUMNS]
    validate_df(df)
    df.to_csv(OUT_DIR / "03_openrouter_outputs_resume.csv", index=False)
    log(f"Saved 03_openrouter_outputs_resume.csv ({len(df)} rows)")

    pd.DataFrame(cost_tracker.history).to_csv(OUT_DIR / "03_openrouter_cost_history_resume.csv", index=False)
    log(f"Saved 03_openrouter_cost_history_resume.csv (${cost_tracker.spent:.4f})")
    ...
    manifest = {
        ...
        "output_hash": sha256(OUT_DIR / "03_openrouter_outputs_resume.csv"),
        "schema_match_modal": True,
        "run_type": "full_817_resume",
        "resume_from": RESUME_OFFSET + 1,
        "original_pilot_rows": 1200,
    }
    with open(OUT_DIR / "03_openrouter_manifest_resume.json", "w", encoding="utf-8") as f:
