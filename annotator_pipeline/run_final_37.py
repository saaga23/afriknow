#!/usr/bin/env python3
"""
Final resume runner: completes the last 37 items (781-817).
Uses 02_sampled_items_final.json.
"""
...
SAMPLED_JSON = OUT_DIR / "02_sampled_items_final.json"
RESUME_OFFSET = 780  # items 0-780 already done
...
def main():
    ...
    log(f"Loaded {len(items)} final items from {SAMPLED_JSON.name}")
    ...
    if DRY_RUN:
        ...
        df.to_csv(OUT_DIR / "03_openrouter_outputs_final.csv", index=False)
        log(f"Saved 03_openrouter_outputs_final.csv ({len(df)} rows, dry-run)")
        return
    ...
    validate_df(df)
    df.to_csv(OUT_DIR / "03_openrouter_outputs_final.csv", index=False)
    log(f"Saved 03_openrouter_outputs_final.csv ({len(df)} rows)")

    pd.DataFrame(cost_tracker.history).to_csv(OUT_DIR / "03_openrouter_cost_history_final.csv", index=False)
    log(f"Saved 03_openrouter_cost_history_final.csv (${cost_tracker.spent:.4f})")
    ...
    manifest = {
        ...
        "output_hash": sha256(OUT_DIR / "03_openrouter_outputs_final.csv"),
        "schema_match_modal": True,
        "run_type": "full_817_final",
        "resume_from": RESUME_OFFSET + 1,
        "original_pilot_rows": 1200,
    }
    with open(OUT_DIR / "03_openrouter_manifest_final.json", "w", encoding="utf-8") as f:
