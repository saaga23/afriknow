#!/usr/bin/env python3
"""
AfriKnow Full Expansion Runner — 817 items × 7 models

Direct copy of 03_openrouter_runner.py configured for the full dataset.
Uses 02_sampled_items_full.json (817 items) instead of the 120-item pilot.
"""
...
SAMPLED_JSON = OUT_DIR / "02_sampled_items_full.json"
...
def log(msg: str) -> None:
    print(f"[full-817] {msg}", flush=True)
...
def main():
    ...
    if DRY_RUN:
        ...
        for idx, it in enumerate(items):
            for model_id, nick, temp, max_tok in active:
                ...
                base = {
                    "item_idx": idx, "id": it.get("id"), "qid": it.get("qid", it.get("id")),
        ...
        df.to_csv(OUT_DIR / "03_openrouter_outputs_full.csv", index=False)
        log(f"Saved 03_openrouter_outputs_full.csv ({len(df)} rows, dry-run)")
        return
        ...
        for model_id, nick, temp, max_tok in active:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    g_ok = True
                    base = {
                        "item_idx": idx, "id": item.get("id"), "qid": item.get("qid", item.get("id")),
    ...
    df = pd.DataFrame(results)
    df.to_csv(OUT_DIR / "03_openrouter_outputs_full.csv", index=False)
    log(f"Saved 03_openrouter_outputs_full.csv ({len(df)} rows)")

    pd.DataFrame(cost_tracker.history).to_csv(OUT_DIR / "03_openrouter_cost_history_full.csv", index=False)
    log(f"Saved 03_openrouter_cost_history_full.csv (${cost_tracker.spent:.4f})")
    ...
    manifest = {
        ...
        "output_hash": sha256(OUT_DIR / "03_openrouter_outputs_full.csv"),
        "schema_match_modal": True,
        "run_type": "full_817_expansion",
        "original_pilot_rows": 1200,
    }
    with open(OUT_DIR / "03_openrouter_manifest_full.json", "w", encoding="utf-8") as f:
